...
@torch.no_grad()
def audio_rec(prompt=None, language='auto', accent='no-accent'):
    ...
    wav_pr, sr = torchaudio.load(prompt, 24000)
    ...
    encoded_frames = tokenize_audio(codec, (wav, sr))
    # print(encoded_frames.shape)
    audio_prompts = encoded_frames[0][0].transpose(2, 1).cpu().numpy()
    audio_prompts = torch.LongTensor(audio_prompts).to(device)
    # import pdb
    # pdb.set_trace()
    # load prompt
    ...
    # frames = encoded_frames.permute(2,0,1) # torch.Size([8, 1, 640])
    frames = audio_prompts.permute(2,0,1) # torch.Size([8, 1, 640])
    # import pdb
    # pdb.set_trace()
